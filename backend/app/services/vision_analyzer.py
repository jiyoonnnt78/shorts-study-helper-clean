...
각 프레임은 "[N초 지점]" 표시와 함께 시간순으로 제공돼.
프레임 하나하나를 실제로 보고, 그 장면에 무엇이 보이는지/어떤 역할인지 구체적으로 적어.
절대 일반론("흥미로운 장면이에요")으로 때우지 말고, 실제 화면 내용을 근거로 써.
훅(hook) 분석은 특히 첫 1~2개 프레임과 제목을 중심으로 판단해.
...

{
  ...
  "audience": ["시청 대상 2~3개"],
  "hook_type": "궁금증 유발형 | 반전형 | 정보 제시형 | 감정 자극형 | 비주얼 압도형 | 리스트·순위형 중 하나",
  "hook_strength": 0~100 사이 정수(초반이 시선을 얼마나 강하게 끄는지),
  "hook_summary": "이 영상이 초반(첫 1~2장면)에 어떻게 관심을 끄는지 한 줄",
  "hook_reason": "왜 그렇게 판단했는지(첫 프레임/제목 근거)",
  "hook_improvement_tip": "더 강한 훅으로 만들려면 어떻게 하면 좋은지 한 줄",
  "persuasion": ["영상이 쓴 설득/몰입 방식 2~4개"],
  "structure": {
    "opening":     {"content": "처음 장면에서 실제로 무엇을 보여주는지", "purpose": "왜 그렇게 시작하는지"},
    "development": {"content": "중간 전개에서 실제로 일어나는 일", "purpose": "그 역할"},
    "climax":      {"content": "가장 인상적인 부분에서 보이는 것", "purpose": "그 역할"},
    "ending":      {"content": "마무리 장면", "purpose": "그 역할"}
  },
  "scenes": [
    {
      "at_sec": 제공된 프레임의 N초(숫자),
      "observation": "이 프레임에 실제로 보이는 것을 구체적으로(인물/사물/자막/배경/행동)",
      "role": "이 장면이 영상에서 맡은 역할(예: 시선 끌기, 정보 전달, 반전, 마무리 등)",
      "purpose": "왜 이 장면을 이 위치에 넣었는지(시청자에게 주는 효과)"
    }
  ],
...
주의: scenes 배열은 제공된 프레임 개수와 같은 수로, 각 프레임에 1:1로 대응해서 시간순으로 작성해.
"""
